...
function draw() {
	if (!buttonP) {
		...
		fillGrid()
		write(
			'You can add or remove live cells, as well as wipe the board. Press enter to run the game.'
		)
	} else if (keyP == false) {
		fillGrid()
		write(
			"Enter live cells. Press 'r' to randomise or 'w' to wipe the board. Press enter to run the game."
		)
		check = true
	} else {
		fillGrid()
		write(
			"Game is running. Press enter again to pause. Press 'w' to wipe the board and start again."
		)
